...
class ImportEngine @Inject constructor(
    ...
    suspend fun importMarkdown(uri: Uri): Boolean {
        return try {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                ...
                val lines = mdContent!!.lines().filter { it.startsWith("|") && !it.startsWith("| ---") && !it.startsWith("| 样本") && !it.startsWith("| 样品") && !it.startsWith("| Sample") }
    ...
    companion object {
        fun parsePosition(label: String): Pair<Int, Int>? {
